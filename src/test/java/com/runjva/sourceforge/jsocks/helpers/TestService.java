...
public class TestService implements Runnable {
	...
	static final String serviceNames[] = { "echo", "discard", "chargen",
			"connect" };
	...
	/**
	 * Maps service name to the integer id, does it in simple linear search
	 * manner.
	 * 
	 * @param serviceName
	...
	/**
	 * Echos any input on the socket to the output. Echo is being done line by
	 * line.
	 * 
	 * @param s
	 *            Socket on which to perform service.
	...
	/**
	 * Reads input from the socket, and does not write anything back. logs input
	 ...
	 * send after increasingly larger time intervals. It starts from wait_time
	 * (ms), and each next time wait time is doubled. Eg. 1 2 4 8 16 32 64 128
	 * 256 512 1024 2048 4096 8192 ... well you got the idea.
	 * <p>
	 * It starts if either connection is clsoed or the wait time grows bigger
	 * than max_wait.
	 * 
	 * @param s
	 *            Socket on which to perform service.
	 ...
	 * service_id. Connects back to the remote host to port specified in the
	 * line, if successful performs a service speciefied by id on that new
	 * connection. If accept was successful closes the control connection, else
	 * outputs error message, and then closes the connection.
	 * 
	 * @param s
	 *            Control connection.
	 */
